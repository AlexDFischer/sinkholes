...
def process_geotiff(geotiff_input_filename: str,
                    geotiff_output_filename: str,
                    sinkholes_output_filename: str,
                    config: Config,
    ...
    color_util = ColorUtil(config.min_depth_for_colormap, config.max_depth_for_colormap,
                           config.pin_colormap, config.map_colormap)
    if config.verbose:
...
parser.add_argument('-odb', '--output-database')
...
if 'config' in args and args.config is not None:
    ...
    try:
        ...
        print(
            f'Error while reading config file "{args.config}". Using default config values. Exception stacktrace:')
